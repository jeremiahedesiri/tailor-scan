...
`mesh-refinement.js` now uses eligible tape anchors as local 3D constraints. It copies the raw mesh, scales only the radial geometry around the relevant anatomical plane, blends the adjustment smoothly along the local anatomical axis, and remeasures the altered mesh. It uses at most four bounded iterations and retains both raw and refined geometry. Constraints requiring more than a 30% local change, or lacking a reliable closed cross-section, are withheld for review rather than applied. Final refined values come from the refined mesh remeasurement—not directly from the tape input.
